document.addEventListener('DOMContentLoaded', () => {
  ...
  // Función para mostrar la imagen expandida y el texto
  function showExpandedImage(src) {
    expandedImgContainer.innerHTML = '';

    // Crear el elemento de imagen expandida
    const expandedImg = document.createElement('img');
    expandedImg.src = src;
    expandedImgContainer.appendChild(expandedImg);

    // Crear el texto "Deslizá para ver más"
  ...
  function closeExpandedImage() {
    expandedImgContainer.innerHTML = ''; // Limpiar contenido
  ...
  let touchStartX = 0;
  let touchEndX = 0;
  const threshold = 50; // Umbral de desplazamiento para considerar un gesto de deslizamiento
  ...
  function handleSwipeGesture() {
    ...
    if (Math.abs(swipeDistance) >= threshold) {
      if (swipeDistance > 0) {
        ...
        // Aquí puedes realizar acciones específicas cuando se desliza hacia la derecha
      } else {
        // Deslizamiento hacia la izquierda
        // Aquí puedes realizar acciones específicas cuando se desliza hacia la izquierda
      }
    }
  }
